...
const createServer = () => {
  ...
  app.get('/', (request,response) => response.status(200).send('I am a Dragon'));
  app.delete ('/things/1', (request,response) => response.status(405).send('the Dragons deleted the thing'));
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  var biggerArr = arr.filter(character => character.mass > 77);
  console.log(biggerArr);

  var names = biggerArr.map(bigChars => {
    return bigChars.name;
  });
  var string = `${names}`;
