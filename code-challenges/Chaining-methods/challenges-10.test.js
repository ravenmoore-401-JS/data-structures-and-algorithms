...
const createServer = () => {
  ...
  const app =express();
  app.get('/hello', (req,res) => res.status(200).send('I am a Dragon'));
  app.get('/aboutme',(req,res) => res.status(200).send('my name is matt Ravenmoore and i am a software developer'));
  app.get('/favoritefoods',(req,res) => {
    let foodArr = ['shepards pie','brisket, slow cooked','key lime pie'];
...
const totalSum = (input) => {
  let newArr = [];
  let lastTotal;
  newArr = input.map((arr => {
    const newNum = arr.reduce((accumulator, currentValue) => accumulator + currentValue);
    return newNum;
  }));
  lastTotal = newArr.reduce((accumulator, currentValue) => accumulator + currentValue);
  return lastTotal;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let secondArr;
  input.map((arr =>{
    firstArr = arr.filter((num)=> num !== ' ' || num === num % 5);
  }));
  console.log(firstArr);
  secondArr = firstArr.map((arr => Math.pow(2, arr)));
